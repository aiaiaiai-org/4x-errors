...
RSpec.describe Aiaiaiai::Errors::Collector::ProductionSmoke, :database do
  ...
  it 'passes against a migrated database' do
    ...
    expect(smoke.checks.map(&:name)).to eq(%w[
                                             ...
                                             health_database_dependency
                                             ingest_test_event read_back_test_event idempotency_test
                                             cross_project_token_rejection
                                             verify_received_at_is_server_generated
                                           ])
    expect(smoke.checks.reject(&:ok)).to be_empty
  end
  ...
  it 'leaves nothing behind' do
    ...
    definitions = database[:error_definitions].where(error_id: described_class::SMOKE_ERROR_ID)
    families = database[:error_families].where(family_id: described_class::SMOKE_FAMILY_ID)

    expect(definitions.count).to be_zero
    expect(families.count).to be_zero
  end
