...
RSpec.describe Aiaiaiai::Errors::App, 'trusted ingest round trip' do
  ...
  it 'authenticates, validates and persists the canonical event' do
    ...
    persisted = database[:error_events].where(event_id: event.fetch('event_id')).first
    expect(last_response.status).to eq(201)
    expect(persisted).to include(
      project: 'nilx-one/web',
      error_id: 'ai.model.load.failed'
    )
    expect(persisted.fetch(:received_at)).not_to be_nil
  end
