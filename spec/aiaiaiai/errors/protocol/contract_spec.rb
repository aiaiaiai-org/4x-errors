...
RSpec.describe 'errors.v1 conformance' do
  fixtures = Dir[File.expand_path('../../../../protocol/fixtures/*/*.json', __dir__)]
  ...
  describe 'the published schema' do
    ...
    it 'names the same relation and evidence vocabulary as the Ruby implementation' do
      relation_types = Aiaiaiai::Errors::Protocol.schema.dig('$defs', 'relation', 'properties',
                                                             'type', 'enum')
      evidence = Aiaiaiai::Errors::Protocol.schema.dig('$defs', 'relation', 'properties',
                                                       'evidence', 'items', 'enum')
  ...
  describe 'similarity and causality' do
    it 'keeps them distinct: similarity is admissible evidence, never sufficient for a cause' do
      ...
      as_correlation = causal['relations'].first.merge('type' => 'correlated_with')
      correlated = causal.merge('relations' => [as_correlation])
