...
RSpec.describe Sequel::Migrator do
  ...
  it 'creates the error_events table with protocol and server metadata columns' do
    columns = database.schema(:error_events).to_h

    expect(columns.keys).to include(
      :event_id, :protocol_version, :error_id, :project, :source, :severity, :message, :full_text,
      :observed_at, :context, :tags, :family_id, :caused_by_event_id, :correlation_id, :received_at
    )
    expect(columns.fetch(:event_id).fetch(:db_type)).to eq('uuid')
    expect(columns.fetch(:received_at).fetch(:allow_null)).to be(false)
  end

  it 'persists an errors.v1-shaped event and assigns received_at' do
    event_id = '00000000-0000-4000-8000-000000000001'

    database[:error_events].insert(
      event_id: event_id,
  ...
  it 'enables row-level security without creating browser-facing policies' do
    row_security = database.fetch(<<~SQL).get(:relrowsecurity)
      SELECT relrowsecurity
      FROM pg_class
      WHERE oid = 'error_events'::regclass
    SQL
    policy_count = database.fetch(<<~SQL).get(:count)
      SELECT COUNT(*) AS count
      FROM pg_policies
      WHERE schemaname = 'public' AND tablename = 'error_events'
    SQL

    expect(row_security).to be(true)
