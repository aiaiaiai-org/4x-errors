...
module Aiaiaiai
  module Errors
    ...
    module Payload
      ...
      class InvalidReport < StandardError
      end

      module_function

      def event(configuration:, error_id:, exception: nil, message: nil, family_id: nil,
                context: {}, tags: {}, severity: nil, component: nil, occurrence_key: nil,
        ...
        add_description(built,
                        family_id: family_id,
                        component: component || configuration.component,
                        message: message, exception: exception, occurrence_key: occurrence_key)
        add_annotations(built, context: context, tags: tags)
        built
      end

      def add_description(built, family_id:, component:, message:, exception:, occurrence_key:)
        built['family_id'] = semantic_id!(family_id, 'family_id') if family_id
        built['component'] = bounded(component, Protocol::Limits::COMPONENT_CHARS) if component
        ...
        if occurrence_key
          built['occurrence_key'] =
            bounded(occurrence_key, Protocol::Limits::ID_CHARS)
        end
        built
      end

      def add_annotations(built, context:, tags:)
      ...
      def bounded(value, limit)
        Protocol::Bounding.truncate(Protocol::Bounding.safe_string(value), limit)
      end
      ...
      def relation(source:, type:, target:, confidence:, evidence:, note: nil)
        ...
        built['note'] = bounded(note, Protocol::Limits::NOTE_CHARS) if note
        built
      end
      ...
      def endpoint(value)
        if value.is_a?(String) || value.is_a?(Symbol)
          return { 'error_id' => semantic_id!(value,
                                              'error_id') }
        end
        ...
        if normalised.key?('error_id')
          return { 'error_id' => semantic_id!(normalised['error_id'],
                                              'error_id') }
        end
