  ...
  module Errors
    module Collector
      ...
      class ProductionSmoke
        ...
        def summary
          checks.all?(&:ok) ? 'all checks passed; nothing was deployed' : 'validation failed'
        end

        private

        def run_checks
          check('database_connectivity') do
            Database.reachable?(db) or raise 'database did not answer'
          end
          check('migrations_are_current') do
            Database.migrations_current?(db) or raise 'migrations are pending'
          end
        ...
        def check(name)
          ...
          checks << Check.new(name, false, "#{e.class}: #{e.message}")
          nil
        end
        ...
        def repeats_resolve_to_one_event(event_id)
          ...
          raise 'a repeat created a second event' unless body.dig('events', 0,
                                                                  'event_id') == event_id
        ...
        def foreign_token_is_refused
          ...
          unless response.status == 403
            raise "a token from another project was answered with #{response.status}"
          end
        ...
        def received_at_is_the_servers(event_id)
          ...
          unless (Time.now.utc - row[:received_at].utc).abs < 600
            raise 'received_at was not generated by the collector'
          end
        ...
        def smoke_events
          db[:error_events].where(project: [SMOKE_PROJECT, OTHER_PROJECT],
                                  occurrence_key: occurrence_key)
        end
        ...
        def remove_smoke_records
          ...
          return unless db[:error_events].where(error_id: SMOKE_ERROR_ID).empty?

          db[:error_family_memberships].where(error_id: SMOKE_ERROR_ID,
                                              family_id: SMOKE_FAMILY_ID).delete
          ...
          out.puts "smoke records could not be removed: #{e.class}"
        end
      end
    end
  end
